# 📊 Individual Progress Scorecard

## Daily Training Log

**Date:** __March 8, 2026_____________ | **Training Day:** ___/180 (or ___/90 for 3-month plan)

### 1. Time Investment

- [X] Training time today: ___2___ hours
- [ ] Goal met? (Yes/No)
- [ ] Consistency streak: __1__ days

**Points:**

- 2+ hours = 10 points
- 1-2 hours = 5 points
- <1 hour = 2 points
- Missed day = 0 points (streak resets)

### 2. Challenge Completion

| Difficulty      | Challenges Solved | Points Earned |
| --------------- | ----------------- | ------------- |
| Easy            | __6__ × 5 pts     | __30_         |
| Medium          | _____ × 15 pts    | _____         |
| Hard            | _____ × 30 pts    | _____         |
| Expert          | _____ × 50 pts    | _____         |
| **Daily Total** |                   | **__30__**     |

### 3. Quality Indicators

- [ ] Created writeup for at least 1 challenge (+10 pts)
- [ ] Reviewed 3+ writeups from others (+5 pts)
- [ ] Learned new technique/tool (+10 pts)
- [ ] Updated cheat sheet (+5 pts)
- [ ] Practiced timed challenge (+5 pts)

**Quality Points Total:** _____

### 4. Category Focus Today

Which categories did you practice?

- [X] Web Exploitation
- [ ] Binary Exploitation / Pwn
- [ ] Cryptography
- [ ] Reverse Engineering
- [ ] Forensics
- [ ] OSINT
- [ ] Other: ___________

**Primary Specialty:** _____ minutes

**Secondary Specialty:** _____ minutes

**Other Categories:** _____ minutes

### 5. Reflection (Qualitative)

**What went well today?**

--- Becoming more proficient with digging through the html codes.

**What challenged you?**

--- The Web-Server challenges. Trying to decode the stuff I found on the html just results in nothing and asking Gemini tells me on decoding with "JSFuck" (What the heck is a JSFuck?). Tried it and it results to nothing.

**Key learning:**

--- 

**Tomorrow's focus:**

---
